...
(async () => {
  ...
  maintenance = parent + '/blobot-maintenance52-' + config.uuid; assert(!fs.existsSync(maintenance)); fs.mkdirSync(maintenance);
  fs.writeFileSync(maintenance + '/cgroup.procs', String(process.pid));
  assert.equal(read('/proc/self/cgroup').trim(), '0::/docker/blobot-maintenance52-' + config.uuid);
...
const fs=require('node:fs'),cp=require('node:child_process'),assert=require('node:assert/strict');
const config=JSON.parse(process.argv[1]),python=process.argv[2],base='/run/blobot-control52-'+config.uuid,view=base+'/root';
