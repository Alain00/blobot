...
const snapshot = 'blobot-rootfs35-fixture:' + uuid, [repository, tag] = candidate.split(':');
const temp = await mkdtemp(join(tmpdir(), 'blobot-rootfs35-fixture-'));
const worker = await readFile(new URL('./35-rootfs-worker.cjs', import.meta.url), 'utf8');
...
const create = async (role, image, cpus, memory) => {
  const name = 'blobot-rootfs35-' + uuid + '-' + role, dir = join(temp, role);
...
try {
  ...
  assert.equal((await boxes()).length, 0, 'Exclusive sbx use required'); assert.equal((await matching('blobot-rootfs35-fixture', uuid)).length, 0);
  ...
  report.rootfsBefore = await seedWorker.request('seedRootfs'); report.before = await seedWorker.request('state');
  ...
  const saved = await matching('blobot-rootfs35-fixture', uuid); assert.equal(saved.length, 1); snapshotId = saved[0].id; report.saved = saved[0];
  ...
  report.rootfsSnapshot = await targetWorker.request('rootfsState');
  report.rootfsSourceReopened = await sourceWorker.request('rootfsState');
  sameState(report.rootfsSourceReopened, report.rootfsBefore);
  report.snapshotRootfsExact = JSON.stringify(report.rootfsSnapshot) === JSON.stringify(report.rootfsBefore);
  report.targetDrift = await targetWorker.request('driftTargetRootfs');
  ...
  assert.deepEqual(report.targetQuietAfterCopy.package, report.before.package);
  report.rootfsAfter = await targetWorker.request('rootfsState'); sameState(report.rootfsAfter, report.rootfsBefore); assert.deepEqual(report.targetQuietAfterCopy.home, report.before.home);
  report.targetResumed = await targetWorker.request('resume'); report.after = await targetWorker.request('state');
  sameState(report.after, report.before);
  await targetWorker.close();
  await stop(target);
  const reopenedTarget = await connect(target, 'target', 'targetReopened');
  report.targetReopenedInventory = await reopenedTarget.request('inventory');
  report.targetReopenedState = await reopenedTarget.request('state'); sameState(report.targetReopenedState, report.before);
  report.targetReopenedRootfs = await reopenedTarget.request('rootfsState'); sameState(report.targetReopenedRootfs, report.rootfsBefore);
  report.targetRun = await reopenedTarget.request('runRestored'); report.targetMutation = await reopenedTarget.request('mutateTarget');
  ...
  report.originalFinalRootfs = await sourceWorker.request('rootfsState'); sameState(report.originalFinalRootfs, report.rootfsBefore);
  await reopenedTarget.close(); await sourceWorker.close();
  report.checks = { ...report.checks, source2Cpu2GiBTarget3Cpu3GiB: true, privateVolumesExact: true, realContainerdRootMeasured: true,
    rootfsPackageAndConffileContentModeOwnerPreserved: true, measuredRootfsMetadataExact: true, candidateStopReopenPreserved: true,
  ...
  for (const entry of [{ owned: snapshotIntended, repo: 'blobot-rootfs35-fixture', tag: uuid, id: snapshotId, ref: snapshot }, { owned: loadedOwned, repo: repository, tag, id: loadedId, ref: candidate }]) {
  ...
  report.cleanup = { errors, remainingBoxes: (await boxes()).filter(b => intended.has(b.name)), remainingSnapshot: await matching('blobot-rootfs35-fixture', uuid), remainingOwnedCandidate: loadedOwned ? await matching(repository, tag) : [] };
  ...
  await writeFile(new URL('./35-rootfs-fixture-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
