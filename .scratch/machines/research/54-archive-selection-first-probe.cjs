...
async function restore(bytes, target, selection) {
  ...
  const p = cp.spawn('tar', [...extract, '-C', target, '-xpf', '-', '-T', '/proc/self/fd/3'], { env, stdio: ['pipe', 'pipe', 'pipe', 'pipe'] });
  let stderrBytes = 0, stdoutBytes = 0, pipeErrors = 0;
  p.stderr.on('data', b => { stderrBytes += b.length; }); p.stdout.on('data', b => { stdoutBytes += b.length; });
  p.stdin.on('error', () => { pipeErrors++; }); p.stdio[3].on('error', () => { pipeErrors++; });
  const closed = new Promise((resolve, reject) => { p.on('error', reject); p.on('close', (code, signal) => resolve({ invoked: true, exitCode: code, signal, stderrBytes, stdoutBytes, pipeErrors })); });
  const timer = setTimeout(() => p.kill('SIGKILL'), 15_000);
  p.stdio[3].end(selection); p.stdin.end(bytes);
