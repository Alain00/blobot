...
const snapshot = 'blobot-rootfs38-fixture:' + uuid, [repository, tag] = candidate.split(':');
const temp = await mkdtemp('/private/tmp/blobot-rootfs38-fixture-');
...
const worker = await readFile(new URL('./38-full-root-worker.cjs', import.meta.url), 'utf8');
...
const create = async (role, image, cpus, memory) => {
  const name = 'blobot-rootfs38-' + uuid + '-' + role, dir = join(temp, role);
...
try {
  ...
  assert.equal((await boxes()).length, 0, 'Exclusive sbx use required'); assert.equal((await matching('blobot-rootfs38-fixture', uuid)).length, 0);
  ...
  const first=await channel.request('fullRoot38'), firstRecords=first.inventory; delete first.inventory; report.first=first; assert.equal(first.summary.errors.length,0); assert.equal(first.summary.count,Object.values(first.summary.types).reduce((a,b)=>a+b,0));
  report.thaw=await channel.request('thaw36');await channel.close();await stop(source);
  const reopened=await connect(source,'source','reopened');
  report.reopenedInventory=await reopened.request('inventory'); report.reopenedQuiescence=await reopened.request('quiesce');report.reopenedFrozen=await reopened.request('freeze36');
  const second=await reopened.request('fullRoot38'), secondRecords=second.inventory;delete second.inventory;report.reopened=second; assert.equal(second.summary.errors.length,0); assert.equal(second.summary.count,Object.values(second.summary.types).reduce((a,b)=>a+b,0));
  report.reopenDifferences=[];
  for(const path of new Set([...Object.keys(firstRecords),...Object.keys(secondRecords)]))if(JSON.stringify(firstRecords[path])!==JSON.stringify(secondRecords[path]))report.reopenDifferences.push({path,before:firstRecords[path],after:secondRecords[path]});
  report.reopenDifferences.sort((a,b)=>a.path.localeCompare(b.path));
  report.reopenArchiveEqual=first.archives[0].sha256===second.archives[0].sha256;
  report.reopenInventoryEqual=report.reopenDifferences.length===0;
  report.reopenedThaw=await reopened.request('thaw36');await reopened.close();
  report.passed = true;
  ...
  for (const entry of [{ owned: snapshotIntended, repo: 'blobot-rootfs38-fixture', tag: uuid, id: snapshotId, ref: snapshot }, { owned: loadedOwned, repo: repository, tag, id: loadedId, ref: candidate }]) {
  ...
  report.cleanup = { errors, remainingBoxes: (await boxes()).filter(b => intended.has(b.name)), remainingSnapshot: await matching('blobot-rootfs38-fixture', uuid), remainingOwnedCandidate: loadedOwned ? await matching(repository, tag) : [] };
  ...
  await writeFile(new URL('./38-full-root-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
