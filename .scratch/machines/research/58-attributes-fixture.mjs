...
assert.equal(process.env.BLOBOT_LIVE_SBX_ATTRIBUTES58, '1', 'Explicit opt-in required');
...
const uuid = randomUUID(), name = 'blobot-maintenance58-' + uuid;
const temp = await mkdtemp('/private/tmp/blobot-maintenance58-');
const env = sbxClientEnvironment(), exec = promisify(execFile);
const worker = await readFile(new URL('./58-attributes-worker.cjs', import.meta.url), 'utf8');
...
const probe = await readFile(new URL('./58-attributes-probe.py', import.meta.url), 'utf8');
const sources = JSON.stringify({ probe });
const report = { date: new Date().toISOString(), uuid, name, candidate, manifest, scope: 'synthetic disposable guest only', commands: [], diskChecks: [], cleanup: {},
  scripts: { workerSha256: createHash('sha256').update(worker).digest('hex'), prepareSha256: createHash('sha256').update(prepare).digest('hex'), probeSha256: createHash('sha256').update(probe).digest('hex') } };
...
try {
  ...
  const kit = { schemaVersion: '2', kind: 'sandbox', name: 'blobot-synthetic-maintenance58',
  ...
  await writeFile(new URL('./58-attributes-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
  console.log(JSON.stringify({ passed: report.passed, error: report.error, guests: report.guests?.map(g => ({code: g.code, completed: g.guest.completed, checks: g.guest.checks})), cleanup: report.cleanup }));
}
