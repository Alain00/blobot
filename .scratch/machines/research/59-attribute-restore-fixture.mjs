...
assert.equal(process.env.BLOBOT_LIVE_SBX_RESTORE59, '1', 'Explicit opt-in required');
...
const uuid = randomUUID(), name = 'blobot-restore59-' + uuid;
const temp = await mkdtemp('/private/tmp/blobot-restore59-');
const env = sbxClientEnvironment(), exec = promisify(execFile);
const worker = await readFile(new URL('./59-attribute-restore-worker.cjs', import.meta.url), 'utf8');
...
const { createSbxArchiveIndex, selectSbxArchiveMembers } = await import('../../../packages/core/dist/machines/sbx/state-archive-index.js');
const { readSbxStateTree } = await import('../../../packages/core/dist/machines/sbx/state-tree.js');
const { receiveSbxStateArchive } = await import('../../../packages/core/dist/machines/sbx/state-restore.js');
const { sbxStateBundleHeader, createSbxStateBundleReader } = await import('../../../packages/core/dist/machines/sbx/state-bundle.js');
const { createSbxStateAttributeCodec, createSbxStateAttributeBackend, SBX_STATE_ATTRIBUTE_HELPER } = await import('../../../packages/core/dist/machines/sbx/state-attributes.js');
const sources = JSON.stringify({ attributes: createSbxStateAttributeBackend.toString(), attributeCodec: createSbxStateAttributeCodec.toString(), attributeScript: SBX_STATE_ATTRIBUTE_HELPER, select: selectSbxArchiveMembers.toString(), receive: receiveSbxStateArchive.toString(), header: sbxStateBundleHeader.toString(), bundle: createSbxStateBundleReader.toString(), verify: createStateArchiveVerifier.toString(), index: createSbxArchiveIndex.toString(), read: readSbxStateTree.toString() });
const report = { date: new Date().toISOString(), uuid, name, candidate, manifest, scope: 'synthetic disposable guest only', commands: [], diskChecks: [], cleanup: {},
  scripts: { workerSha256: createHash('sha256').update(worker).digest('hex'), prepareSha256: createHash('sha256').update(prepare).digest('hex'), definitionsSha256: createHash('sha256').update(sources).digest('hex') } };
...
try {
  ...
  const kit = { schemaVersion: '2', kind: 'sandbox', name: 'blobot-synthetic-restore59',
  ...
  await writeFile(new URL('./59-attribute-restore-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
