...
- **`06`, `07`, `14`** are unaffected in substance and merely scoped per server.
...
- **Ticket 14's cancellation rule changes meaning.** *With nobody listening a permission request
...
The per-agent composition root (`14`). That axis is per agent under **both** models and does not
wait on this.
