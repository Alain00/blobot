...
try {
  ...
  report.rootfs = await channel.request('seedRootfs');
  report.quiescence = await channel.request('quiesce');
  report.namespace = await channel.request('namespaceProbe');
  await channel.close();
  report.passed = true;
  ...
  await writeFile(new URL('./35-rootfs-namespace-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
