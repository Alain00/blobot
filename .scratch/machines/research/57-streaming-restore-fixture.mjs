...
assert.equal(process.env.BLOBOT_LIVE_SBX_RESTORE57, '1', 'Explicit opt-in required');
...
const uuid = randomUUID(), name = 'blobot-restore57-' + uuid;
const temp = await mkdtemp('/private/tmp/blobot-restore57-');
const env = sbxClientEnvironment(), exec = promisify(execFile);
const worker = await readFile(new URL('./57-streaming-restore-worker.cjs', import.meta.url), 'utf8');
...
const sources = JSON.stringify({ select: selectSbxArchiveMembers.toString(), receive: receiveSbxStateArchive.toString(), header: sbxStateBundleHeader.toString(), bundle: createSbxStateBundleReader.toString(), verify: createStateArchiveVerifier.toString(), index: createSbxArchiveIndex.toString(), read: readSbxStateTree.toString() });
...
try {
  ...
  const kit = { schemaVersion: '2', kind: 'sandbox', name: 'blobot-synthetic-restore57',
  ...
  await writeFile(new URL('./57-streaming-restore-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
