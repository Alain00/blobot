...
describe('commitWorktree', () => {
  ...
  it('takes a selection with a pathspec and never touches the index', async () => {
    const ran: string[] = [];
    const outcome = await commitWorktree(
      { path: '/w', agentName: 'Alice', message: 'one', paths: ['src/a.ts', 'src/b.ts'] },
      async (command, args) => {
        ran.push([command, ...args].join(' '));
        return { code: 0, stdout: 'ab12cd3\n', stderr: '' };
      },
  ...
  it('adds the new files in a selection, because a pathspec cannot name them', async () => {
    ...
    await commitWorktree(
      { path: '/w', agentName: 'Alice', message: 'one', paths: ['src/a.ts', 'src/new.ts'], untracked: ['src/new.ts'] },
      async (command, args) => {
        ran.push([command, ...args].join(' '));
        return { code: 0, stdout: 'ab12cd3\n', stderr: '' };
      },
  ...
  it('takes that add back when the commit is refused', async () => {
    ...
    const ran: string[] = [];
    const outcome = await commitWorktree(
      { path: '/w', agentName: 'Alice', message: 'one', paths: ['src/new.ts'], untracked: ['src/new.ts'] },
      async (command, args) => {
        const key = [command, ...args].join(' ');
        ran.push(key);
  ...
  it('refuses a selection of nothing rather than committing everything', async () => {
    let ran = false;
    const outcome = await commitWorktree({ path: '/w', agentName: 'Alice', message: 'one', paths: [] }, async () => {
      ran = true;
      return { code: 0, stdout: '', stderr: '' };
    });
    expect(ran).toBe(false);
