...
assert.equal(process.env.BLOBOT_LIVE_SBX_SELECTIVE52, '1', 'Explicit opt-in required');
...
const uuid = randomUUID(), name = 'blobot-selective52-' + uuid;
const temp = await mkdtemp('/private/tmp/blobot-selective52-');
const env = sbxClientEnvironment(), exec = promisify(execFile);
const worker = await readFile(new URL('./52-selective-incremental-worker.cjs', import.meta.url), 'utf8');
const python = await readFile(new URL('./52-selective-incremental.py', import.meta.url), 'utf8');
...
try {
  ...
  const kit = { schemaVersion: '2', kind: 'sandbox', name: 'blobot-synthetic-selective52',
  ...
  assert(r.completed && r.cleanup.syntheticTreeRemoved && r.archive.repeatIdentical);
  assert.equal(Object.keys(r.cases).length, 5);
  ...
  await writeFile(new URL('./52-selective-incremental-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
  console.log(JSON.stringify({ passed: report.passed, error: report.error, checks: Object.fromEntries(Object.entries(report.guest?.probe?.result?.cases ?? {}).map(([name, c]) => [name, { exitCode: c.exitCode, checks: c.checks }])), cleanup: report.cleanup }));
}
