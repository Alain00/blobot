...
try {
  ...
  report.quiescence = await channel.request('quiesce');
  report.freezer = await channel.request('freezerProbe');
  report.crash = await channel.request('freezeAndExit');
  ...
  report.recoveredFreezer = await reopened.request('freezerRecovery');
  ...
  await writeFile(new URL('./35-rootfs-freezer-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
