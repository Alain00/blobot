...
Raised by the author, 2026-09-04, as what survives the fork in `13`: *"where machines stands are
...
axis is orthogonal to where the box is, which is why this ticket does not wait on `13`.
...
1. **The noun, and it is probably not *Machine*.** A composition root is not a place. Under `13`'s
