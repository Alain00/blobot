#!/usr/bin/env node
// Native build acceptance, without sign-in or inference. Only --version, a UID/home probe and ACP initialize.
...
if (!['arm64', 'amd64'].includes(arch)) throw new Error('Unsupported architecture');
...
if (dockerHost && !dockerHost.startsWith('unix:///')) throw new Error('Only a local Docker socket is allowed');
const dockerEnv = { ...process.env, ...(dockerConfig ? { DOCKER_CONFIG: dockerConfig } : {}), ...(dockerHost ? { DOCKER_HOST: dockerHost } : {}) };
const request = {
...
async function runContainer(runtime, receipt, phase, executable, commandArgs, acp = false) {
  ...
  const argv = [
    ...
    '--label', 'blobot.fixture=runtime-image-build-smoke',
...
for (const runtime of runtimes) {
  const receipt = JSON.parse(await readFile(join(receipts, `${runtime}-${arch}.json`), 'utf8'));
  if (receipt.runtime !== runtime || receipt.arch !== arch || !/^sha256:[0-9a-f]{64}$/.test(receipt.imageId)) {
  ...
  const [identity, version, initialize] = entry.checks;
  const who = JSON.parse(identity.stdout);
  const response = initialize.initializeResponse;
  const nativeInit = runtime === 'fx'
    ? response?.error?.code === -32600 && /auth|login|sign.?in|API.key/i.test(response.error.message)
    : response?.result?.protocolVersion === 1;
  const bridgeVersion = runtime === 'claude' ? '0.70.0' : runtime === 'codex' ? '1.7.0' : undefined;
  entry.accepted = identity.exitCode === 0 && who.uid === 1000 && who.gid === 1000 &&
    who.home === '/home/agent' && who.homeEntries.length === 0 && who.node === 'v22.22.1' &&
    version.exitCode === 0 && version.stdout.includes(receipt.version) && nativeInit &&
    (bridgeVersion === undefined || response?.result?.agentInfo?.version === bridgeVersion) &&
    (initialize.exitCode === 0 || (runtime === 'cursor' && initialize.timedOut));
...
if (report.results.some(entry => !entry.accepted)) process.exitCode = 1;
