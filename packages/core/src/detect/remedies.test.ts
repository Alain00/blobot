...
describe('remediesFor', () => {
  ...
  it('offers Cursor’s own install and login, named for the distinctive binary', () => {
    ...
    expect(signIn[0]?.argv).toEqual(['/home/dev/.local/bin/cursor-agent', 'login']);
    expect(signIn[0]?.shown).toBe('cursor-agent login');
  });
});
