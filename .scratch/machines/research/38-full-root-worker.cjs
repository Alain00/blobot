...
const fs=require('node:fs'),cp=require('node:child_process'),assert=require('node:assert/strict');
const config=JSON.parse(process.argv[1]),python=process.argv[2],base='/run/blobot-control38-'+config.uuid,view=base+'/root';
...
try {
  ...
  const probes=config.syntheticMounts.map(p=>({path:p,visible:fs.readFileSync(p+'/marker','utf8')==='synthetic-'+config.uuid,rootViewMarker:fs.existsSync(view+p+'/marker')}));
  for(const p of probes){assert(p.visible);assert.equal(p.rootViewMarker,false);}
  const result=JSON.parse(cp.execFileSync('chroot',[view,'/usr/bin/python3','-c',python],{encoding:'utf8',timeout:120000,maxBuffer:32*1024**2,env:{PATH:'/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',LC_ALL:'C',TZ:'UTC'}}));
  console.log(JSON.stringify({namespace:fs.readlinkSync('/proc/self/ns/mnt'),viewMounts:children,syntheticMounts:probes,mounts,...result}));
}finally{if(mounted)cp.execFileSync('umount',[view]);fs.rmdirSync(view);fs.rmdirSync(base);}
`;
  const result=JSON.parse(exec('unshare',['--mount','--propagation','private','/usr/bin/node','-e',script,JSON.stringify(config),rootView38Python],{timeout:150000,maxBuffer:32*1024**2}));
