...
(async () => {
  ...
  maintenance = parent + '/blobot-maintenance50-' + config.uuid; assert(!fs.existsSync(maintenance)); fs.mkdirSync(maintenance);
  fs.writeFileSync(maintenance + '/cgroup.procs', String(process.pid));
  assert.equal(read('/proc/self/cgroup').trim(), '0::/docker/blobot-maintenance50-' + config.uuid);
...
const fs=require('node:fs'),cp=require('node:child_process'),assert=require('node:assert/strict');
const config=JSON.parse(process.argv[1]),python=process.argv[2],base='/run/blobot-control50-'+config.uuid,view=base+'/root';
...
try {
 ...
 const result=JSON.parse(cp.execFileSync('/usr/bin/python3',['-B','-c',python,view,config.uuid],{encoding:'utf8',timeout:30000,maxBuffer:2*1024**2,env:{PATH:'/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',LC_ALL:'C',TZ:'UTC'}}));
 console.log(JSON.stringify({namespace:fs.readlinkSync('/proc/self/ns/mnt'),rootMount,runMount,viewMounts:children,result}));
