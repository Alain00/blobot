...
if (process.env.BLOBOT_LIVE_SBX_ROOTFS !== '1') throw new Error('Explicit opt-in required');
...
const snapshot = 'blobot-freezer36-fixture:' + uuid, [repository, tag] = candidate.split(':');
const temp = await mkdtemp(join(tmpdir(), 'blobot-freezer36-fixture-'));
const worker = await readFile(new URL('./36-sibling-freezer-worker.cjs', import.meta.url), 'utf8');
...
const create = async (role, image, cpus, memory) => {
  const name = 'blobot-freezer36-' + uuid + '-' + role, dir = join(temp, role);
...
try {
  ...
  assert.equal((await boxes()).length, 0, 'Exclusive sbx use required'); assert.equal((await matching('blobot-freezer36-fixture', uuid)).length, 0);
  ...
  report.seed = await channel.request('seed');
  report.rootfs = await channel.request('seedRootfs'); report.baselineState = await channel.request('state');
  report.host={platform:process.platform,arch:process.arch}; assert.equal(process.platform,'darwin');
  report.scope = await channel.request('originalScope36'); assert.equal(report.scope.kernel.platform,'linux');
  // First measure the actual sbx exec placement before any cgroup mutation.
  report.execBefore = JSON.parse(await run(['exec','-u','0',source.name,'/usr/bin/node','-e',"const fs=require('node:fs');console.log(JSON.stringify({pid:process.pid,cgroup:fs.readFileSync('/proc/self/cgroup','utf8').trim()}))"], 'exec-placement-before'));
  assert.equal(report.execBefore.cgroup,'0::'+report.scope.relative);
  // Quiesce Docker after measuring exec placement, in this same channel.
  report.beforeFreezeQuiescence = await channel.request('quiesce');
  report.writer = await channel.request('startWriter36');
  report.frozen = await channel.request('freeze36');
  const marker='/tmp/blobot-exec36-'+uuid, started=performance.now();
  concurrent=spawn('sbx',['exec','-u','0',source.name,'/usr/bin/node','-e',"const fs=require('node:fs');const v={pid:process.pid,cgroup:fs.readFileSync('/proc/self/cgroup','utf8').trim()};fs.writeFileSync(process.argv[1],JSON.stringify(v));console.log(JSON.stringify(v));",marker],{env,stdio:['ignore','pipe','pipe']});
  let output='',error='',done=false;
  concurrent.stdout.on('data',d=>{output+=d;assert(output.length<8192);});concurrent.stderr.on('data',d=>{error=(error+d).slice(-8192);});
  const concurrentDone=new Promise((resolve,reject)=>{concurrent.on('error',reject);concurrent.on('close',code=>{done=true;resolve({code,ms:performance.now()-started,stdout:output,stderr:error});});});
  await new Promise(r=>setTimeout(r,1200));
  report.concurrentWhileFrozen=done?await concurrentDone:{pending:true};
  report.frozenObservation = await channel.request('observeFrozen36');
  if(done)assert.notEqual(report.concurrentWhileFrozen.code,0,'Exec reported success while container frozen');
  report.thaw = await channel.request('thaw36');
  if(report.concurrentWhileFrozen.pending) {
    report.concurrent = await Promise.race([concurrentDone,new Promise((_,reject)=>setTimeout(()=>reject(new Error('Concurrent exec did not complete after thaw')),15000).unref())]);
    assert.equal(report.concurrent.code,0,report.concurrent.stderr); assert.equal(JSON.parse(report.concurrent.stdout).cgroup,'0::'+report.scope.relative);
  } else {
    report.retryAfterThaw=JSON.parse(await run(['exec','-u','0',source.name,'/usr/bin/node','-e',"const fs=require('node:fs');const v={pid:process.pid,cgroup:fs.readFileSync('/proc/self/cgroup','utf8').trim()};fs.writeFileSync(process.argv[1],JSON.stringify(v));console.log(JSON.stringify(v));",marker],'retry-exec-after-thaw'));
    assert.equal(report.retryAfterThaw.cgroup,'0::'+report.scope.relative);
  }
  concurrent=null;
  report.writerStopped = await channel.request('stopWriter36');
  report.postThawState = await channel.request('state'); sameState(report.postThawState,report.baselineState);
  report.preCrashQuiescence = await channel.request('quiesce');
  report.crash = await channel.request('freezeAndExit36');
  report.maintenanceExit = await channel.exited; await channel.pump; assert.equal(report.maintenanceExit.code,75); channels.delete(channel);
  await stop(source);
  const reopened = await connect(source, 'source', 'recovery');
  report.recoveredInventory = await reopened.request('inventory');
  report.recoveredFreezer = await reopened.request('recovery36');
  report.recoveredRootfs = await reopened.request('rootfsState'); sameState(report.recoveredRootfs, report.rootfs);
  report.recoveredState = await reopened.request('state'); sameState(report.recoveredState, report.baselineState);
  ...
  for (const entry of [{ owned: snapshotIntended, repo: 'blobot-freezer36-fixture', tag: uuid, id: snapshotId, ref: snapshot }, { owned: loadedOwned, repo: repository, tag, id: loadedId, ref: candidate }]) {
  ...
  report.cleanup = { errors, remainingBoxes: (await boxes()).filter(b => intended.has(b.name)), remainingSnapshot: await matching('blobot-freezer36-fixture', uuid), remainingOwnedCandidate: loadedOwned ? await matching(repository, tag) : [] };
  ...
  await writeFile(new URL('./36-sibling-freezer-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
