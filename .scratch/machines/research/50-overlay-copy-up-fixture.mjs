...
assert.equal(process.env.BLOBOT_LIVE_SBX_COPYUP, '1', 'Explicit opt-in required');
...
const uuid = randomUUID(), name = 'blobot-copyup50-' + uuid;
const temp = await mkdtemp('/private/tmp/blobot-copyup50-');
const env = sbxClientEnvironment(), exec = promisify(execFile);
const worker = await readFile(new URL('./50-overlay-copy-up-worker.cjs', import.meta.url), 'utf8');
const python = await readFile(new URL('./50-overlay-copy-up.py', import.meta.url), 'utf8');
const report = { date: new Date().toISOString(), uuid, name, candidate, manifest, scope: 'synthetic disposable guest only', commands: [], diskChecks: [], cleanup: {},
  scripts: { workerSha256: createHash('sha256').update(worker).digest('hex'), pythonSha256: createHash('sha256').update(python).digest('hex') } };
...
try {
  ...
  const kit = { schemaVersion: '2', kind: 'sandbox', name: 'blobot-synthetic-copyup50',
  ...
  assert(r.completed); assert(Object.values(r.checks).every(Boolean));
  assert(r.operations.openForWriteWithoutWriting.ok && r.operations.appendBeforeExplicitImmutable.ok);
  assert.equal(r.operations.appendWithExplicitImmutable.errno, 1); assert.equal(r.operations.unlinkWithExplicitImmutable.errno, 1);
  for (const key of ['afterOpenWithoutWrite', 'afterSameModeChmodFile', 'afterSameModeChmodDirectory', 'afterRecreateIdenticalBytesAndTimes']) {
    assert.equal(r[key].getflags.flags, 0x80000); assert.equal(r[key].fsgetxattr.xflags, 0); assert.equal(r[key].statx.attributes & 0x10, 0);
  }
  ...
  await writeFile(new URL('./50-overlay-copy-up-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
  console.log(JSON.stringify({ passed: report.passed, error: report.error, checks: report.guest?.probe?.result?.checks, cleanup: report.cleanup }));
}
